...
func (w *writer) instruction(in ir.Instruction) {
	...
	case *ir.ShuffleVector:
		w.simpleInstruction3("shuffleelement ", in.Value1, in.Value2, in.Mask)
	...
	case *ir.GetElementPtrDyn:
		...
		if !utils.IsNil(in.ElementIndex) {
			w.string(", ")
	...
	case *ir.FpToInt:
		w.toInstruction(utils.Ternary(in.Signed, "fptosi ", "fptoui "), in.Value, in.Typ)

	case *ir.IntToFp:
		w.toInstruction(utils.Ternary(in.Signed, "sitofp ", "uitofp "), in.Value, in.Typ)
	...
	case *ir.ICmp:
		w.cmpInstruction("icmp ", utils.Ternary(in.Signed, "s", "u"), in.Op, in.Left, in.Right)

	case *ir.FCmp:
		w.cmpInstruction("fcmp ", utils.Ternary(in.Ordered, "o", "u"), in.Op, in.Left, in.Right)
